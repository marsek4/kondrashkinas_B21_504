...
def main():
    ...
    with open(os.path.join(output_path, 'result.txt'), 'w') as res_file:
        for humiliation, voice_path in humiliation_voice_paths:
            ...
            res_file.write(f"\tThe most timbre-colored fundamental tone: {get_max_tembr(voice_path)}. "
                           "This is the frequency for which the largest number of overtones can be traced\n")
            if 'letter' in humiliation:
                res_file.write(f"\tThe three strongest formants: {get_peaks(freq, t, spec)}. "
                               "These are the frequencies with the highest energy in a certain vicinity\n")
